...
class PosDatasetReader(DatasetReader):
    ...
    def __init__(self, token_indexers: Dict[str, TokenIndexer] = None, granularity: str = '10-class') -> None:
        super().__init__(lazy=False)
        self.token_indexers = token_indexers or {'tokens': SingleIdTokenIndexer()}
        self.granularity = granularity

    def text_to_instance(self, tokens_text: List[Token], label: int = None) -> Instance:
        text_field = TextField(tokens_text, self.token_indexers)
        fields = {'text': text_field}

        if label is not None:
            if self.granularity == '5-class':
                label = round(label/2)
            elif self.granularity == '3-class':
                if label < 3:
                    label = 0
                elif label > 6:
                    label = 2
...
if __name__ == '__main__':
    ...
    reader = PosDatasetReader(granularity='10-class')
