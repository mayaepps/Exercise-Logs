...
class PosDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        for row_index, row in data.iterrows():
            ...
            match = row['match']
            yield self.text_to_instance([Token(char) for char in text], [Token(char) for char in label], match)

### NN model structure
class Net(Model):

    def __init__(self, char_embeddings: TextFieldEmbedder, encoder: Seq2SeqEncoder, vocab: Vocabulary, padding: int) -> None:
    ...
    def forward(self, text: Dict[str, torch.Tensor], label: Dict[str, torch.Tensor], match: torch.Tensor = None) -> Dict[str, torch.Tensor]:
        ...
        text_embeddings = (self.char_embeddings(text))
        label_embeddings = (self.char_embeddings(label))
...
if __name__ == "__main__":
    ...
    lstm = PytorchSeq2SeqWrapper(torch.nn.LSTM(EMBEDDING_DIM, HIDDEN_DIM, batch_first = True, num_layers = 1)) #LSTM or GRU and num_layers = #

    max_length = 260 #260 for sentence and 60 for exercise
